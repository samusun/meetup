...
export default function Rate({ eventName }: Props): ReactElement {
  ...
  function getAllEvents() {
    let mergedList: any = [];
    localEvents
      ? (mergedList = eventData.concat(JSON.parse(localEvents)))
      : (mergedList = eventData);
  ...
  return (
    <>
        ...
        <form className='rating'>
          <label>
            <input
              ...
              name='stars'
              value='1'
              onClick={(event) => sendRating(event.target)}
            />
            <span data-testid='rate3' className='icon'>
              ★
            </span>
          </label>
          <label>
      ...
      {average && (
        <p data-testid='ratingResponse' className='ratingResponse'>
          {ratingResponse}
        </p>
      )}
    </>
