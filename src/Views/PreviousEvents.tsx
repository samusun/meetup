  ...
  useEffect(() => {
    ...
    return (
        <div className="container" >
            {onlyPreviousEvents && onlyPreviousEvents.map((data: any, index) => (
          <Event key={index} eventName = {data.eventName} date= { data.date} time = {data.time} place = {data.place} description = {data.description} participants = { data.participants} participantsMax = {data.participantsMax} rating = {data.rating} comments = {data.comments} previous = {true} />
            ))}
    </div>
    )
}
